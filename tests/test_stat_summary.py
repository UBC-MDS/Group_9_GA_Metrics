...
# Test for correct return value of mean
def test_mean():
    assert output.iloc[0, 0] == (0.9 + 0.8) /2, f"The mean value is wrong with test data: {test_data_path}"

# Test for correct return value of standard deviation
def test_std():
...
# Test for the raised error when there is unexpected input type
def test_input_type_error():
    with pytest.raises(TypeError):
        stat_summary('data', campaign_id, start_date, end_date)

# Test for correct handling for situation where all values are zeros in 'totals.visits', to avoid devided by zero.
def test_all_zero():
